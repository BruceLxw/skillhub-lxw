  ...
  return (
    <>
      ...
      <main ref={heroView.ref} className={`relative z-10 flex flex-col items-center pt-16 pb-20 px-4 md:pt-24 scroll-fade-up${heroView.inView ? ' in-view' : ''}`}>
        <h1 className="text-5xl md:text-7xl font-bold tracking-tight text-[#212529] mb-4">
          技能管理平台
        </h1>
        <h2
      ...
      {/* Popular + Latest side by side */}
      <div className="w-full py-20 md:py-24 px-6" style={{ background: 'var(--bg-page, hsl(var(--background)))' }}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Popular Downloads */}
          <section ref={popularView.ref} className={`scroll-fade-up${popularView.inView ? ' in-view' : ''}`}>
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm" style={{ color: 'hsl(var(--text-secondary))' }}>{t('home.popularDescription')}</p>
              </div>
              {isLoadingPopular ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                  {popularSkills?.items.map((skill, idx) => (
                    <div key={skill.id} className={`animate-fade-up delay-${Math.min(idx + 1, 3)}`}>
                      <SkillCard
                        skill={skill}
          ...
          {/* Latest Releases */}
          <section ref={latestView.ref} className={`scroll-fade-up${latestView.inView ? ' in-view' : ''}`}>
            <div className="space-y-4">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h2 className="text-2xl font-bold tracking-tight" style={{ color: 'hsl(var(--foreground))' }}>
                ...
                <p className="text-sm" style={{ color: 'hsl(var(--text-secondary))' }}>{t('home.latestDescription')}</p>
              </div>
              {isLoadingLatest ? (
                <SkeletonList count={3} />
              ) : (
